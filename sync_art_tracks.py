...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            page.wait_for_selector("a#video-title, a.yt-simple-endpoint", timeout=8000)
        ...
        for i, v_id in enumerate(batch):
            try:
                ...
                # Use 'commit' load state instead of 'domcontentloaded' to parse instantly without waiting for extra network assets
                page.goto(video_url, wait_until="commit")
                
                # Aggressively fast timeout and minimal delay
                page.wait_for_selector("ytd-watch-metadata", timeout=3000)
                ...
                if "Provided to YouTube by" not in description and "Auto-generated by YouTube" not in description:
                    print(f"Skipping {v_id}: Not an official Art Track.")
                    continue
                ...
                if playlist_option.is_visible():
                    ...
                    print(f"Successfully added official Art Track {v_id}.")
                    added_tracks.add(v_id)
                    added_count += 1
                    page.wait_for_timeout(300)
                else:
                    print(f"Could not locate playlist identifier.")
