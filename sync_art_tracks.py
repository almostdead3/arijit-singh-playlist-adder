    ...
    with sync_playwright() as p:
            ...
            if "playlist?list=" in link or "list=" in link:
                ...
                try:
                    ...
                    playlist_links = page.eval_on_selector_all(
                        "ytd-playlist-video-renderer a[href*='/watch?v='], a.yt-simple-endpoint[href*='/watch?v=']",
                        "elements => elements.map(e => e.href)"
                    )
                ...
                if "/watch?v=" in link and "index=" not in link:
                    v_id = link.split("watch?v=")[1].split("&")[0]
                    if v_id not in expanded_video_ids:
                        expanded_video_ids.append(v_id)

        print(f"Total track IDs collected without verification: {len(expanded_video_ids)}")
        ...
        for i, v_id in enumerate(batch):
            try:
                ...
                print(f"[{i + 1}/{len(batch)}] Directly adding track: {v_id}")
                page.goto(video_url, wait_until="domcontentloaded")
                page.wait_for_timeout(random.randint(1000, 1800)) # Super fast safe delay
                ...
                save_btn.click()
                page.wait_for_timeout(1500) # Wait for popup render
                ...
                if playlist_option.is_visible():
                    ...
                    print(f"Successfully added track {v_id} to playlist.")
        ...
        print(f"Batch completed! Successfully added {added_count} tracks to your playlist.")
        browser.close()
