...
def main():
    ...
    with sync_playwright() as p:
        ...
        for link in all_links:
            # Only process direct release/album containers or clean watch links to avoid side-panel junk
            if "playlist?list=" in link or "list=" in link:
                ...
                # Ensure we only grab items from actual release grids, ignoring sidebar recommended side-effects
                if "/watch?v=" in link and "index=" not in link:
                    v_id = link.split("watch?v=")[1].split("&")[0]
                    if v_id not in expanded_video_ids:
                        expanded_video_ids.append(v_id)

        print(f"Filtered clean track IDs collected: {len(expanded_video_ids)}")
        ...
        for i, v_id in enumerate(batch):
            try:
                ...
                page.wait_for_timeout(random.randint(1500, 2500)) # Shortened safe wait time
